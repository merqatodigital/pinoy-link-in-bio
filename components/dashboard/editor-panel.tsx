...
const fieldClass = "mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10";
...
function SectionHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <header className="mb-7">
      ...
      <h1 className="mt-2 text-2xl font-black tracking-[-0.035em] text-slate-950 sm:text-3xl">{title}</h1>
  ...
  return (
    <>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3">
        <button type="button" onClick={() => onAddBlock("product")} className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-slate-950/10">＋ Add product or service</button>
        <button type="button" onClick={() => onAddBlock("link")} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 hover:bg-slate-50">＋ Add simple link</button>
      </div>
      <div className="space-y-4">
        {store.blocks.map((block, index) => (
          <article key={block.id} className={`rounded-[24px] border bg-white p-4 shadow-sm transition ${block.visible ? "border-slate-200" : "border-dashed border-slate-300 opacity-65"}`}>
  ...
  return (
    <>
      ...
      <div className="mt-8 rounded-[24px] border border-rose-200 bg-rose-50 p-5">
        ...
        <button type="button" onClick={onReset} className="mt-4 rounded-full bg-rose-700 px-4 py-2.5 text-xs font-bold text-white">Restore demo data</button>
      </div>
    </>
  );
}
